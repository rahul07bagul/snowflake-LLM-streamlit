...
def get_table_context(table_name: str, table_description: str, metadata_query: str = None):
    ...
    columns = conn.query(f"""
        ...
        WHERE TABLE_SCHEMA = '{table[1].upper()}' AND TABLE_NAME = '{table[2].upper()}' LIMIT 10
        """, show_spinner=False,
    )
    columns = "\n".join(
        [
...
def get_system_prompt(SESSION, SCHEMA_PATH, TABLE_NAME):
    METADATA_QUERY = f"SELECT VARIABLE_NAME, DEFINITION FROM {SCHEMA_PATH}.{TABLE_NAME} LIMIT 10;"
    table_context = get_table_context(
        table_name=SCHEMA_PATH+"."+TABLE_NAME,
        table_description=TABLE_DESCRIPTION,
        metadata_query=METADATA_QUERY
    )
    return GEN_SQL.format(context=table_context)

def create_chart_prompt(messages):
    plot_flag = False
    #print(messages)
    if "plot" in messages[-1]["content"].lower():
        plot_flag = True
        code_prompt = """
            Generate the code for plotting the previous data or {data} 
            but always use some type of data as per request,
        ...
        for message in messages:
            # Check if "results" key is in the current message
            if "results" in message:
                latest_result = message["results"]  # Update latest_result with the current message's results
        if latest_result is not None:
            #print(type(latest_result))
            code_prompt = code_prompt.format(data="use below data \n: " + latest_result.to_string())
        else:
